...
<section style="background: #FFFFFF; width: 100%; min-height: 100vh;">
        ...
        {{-- ======================================================= --}}
                    <div class="col-md-12">
                        <!--begin::Quick Example-->
                        <form action="{{ route('update.datatkkkbbenw', $data->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
    ...
    <div class="row">
        ...
        <div class="col-md-6">
            ...
            <div class="form-modern mb-3">
                <label class="form-label-modern" for="skkanda">
                    <i class="bi bi-image" style="margin-right: 10px; color: navy;"></i> SKK / Foto
                </label>

                <!-- Preview jika ada -->
                @if($data->skkanda && file_exists(public_path('storage/' . $data->skkanda)))
                    <img src="{{ asset('storage/' . $data->skkanda) }}" alt="SKK" style="width: 100%; max-height: 250px; object-fit: contain;">
                @endif
            ...
            <div class="form-modern mb-3">
                ...
                <select id="agendaskk_id" name="agendaskk_id" class="form-control @error('agendaskk_id') is-invalid @enderror">
                    <option value="">Pilih Agenda SKK</option>
                    @foreach($dataagendaskk as $agenda)
                        <option value="{{ $agenda->id }}" {{ old('agendaskk_id', $data->agendaskk_id) == $agenda->id ? 'selected' : '' }}>
                            {{ $agenda->namakegiatan }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-modern mb-3">
                ...
                <input type="text" id="nik" name="nik" class="form-control @error('nik') is-invalid @enderror" value="{{ old('nik', $data->nik) }}" />
                @error('nik')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div class="col-md-6">
            ...
            <div class="form-modern mb-3">
                ...
                <select id="kecamatankbb_id" name="kecamatankbb_id" class="form-control @error('kecamatankbb_id') is-invalid @enderror">
                    <option value="">Pilih Kecamatan</option>
                    @foreach($datakecamatankbb as $kec)
                        <option value="{{ $kec->id }}" {{ old('kecamatankbb_id', $data->kecamatankbb_id) == $kec->id ? 'selected' : '' }}>
                            {{ $kec->kecamatankbb }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-modern mb-3">
                ...
                <select id="tahunpilihan_id" name="tahunpilihan_id" class="form-control @error('tahunpilihan_id') is-invalid @enderror">
                    <option value="">Pilih Tahun</option>
                    @foreach($datatahunpilihan as $tahun)
                        <option value="{{ $tahun->id }}" {{ old('tahunpilihan_id', $data->tahunpilihan_id) == $tahun->id ? 'selected' : '' }}>
                            {{ $tahun->tahunpilihan }}
                        </option>
                    @endforeach
                </select>
...
<div style="display: flex; align-items: center; text-align: center; margin: 10px 0;">
    ...
    <!-- File Upload Section (preview + new upload) -->
    <div class="row mt-4">
        @php
            $fileFields = [
...
@foreach($fileFields as $field => $info)
    <div class="col-md-4 mb-3">
        <label class="form-label-modern" for="{{ $field }}">
            <i class="bi {{ $info['icon'] }}" style="margin-right: 8px; color: navy;"></i> {{ $info['label'] }}
        </label>
        ...
        <div style="margin-top: 10px;">
            ...
            @if($file && file_exists(public_path('storage/' . $file)))
                ...
                @if(in_array($ext, ['jpg','jpeg','png','gif','webp']))
                    <img src="{{ asset('storage/' . $file) }}" alt="{{ $info['label'] }}" style="width:100%; max-height:300px; object-fit:contain;" loading="lazy">
                @elseif($ext === 'pdf')
                    <iframe src="{{ asset('storage/' . $file) }}" frameborder="0" width="100%" height="300px" style="border:1px solid #ccc;"></iframe>
                @else
            ...
            @elseif($file)
                @php $ext = strtolower(pathinfo($file, PATHINFO_EXTENSION)); @endphp
                @if(in_array($ext, ['jpg','jpeg','png','gif','webp']))
                    <img src="{{ asset($file) }}" alt="{{ $info['label'] }}" style="width:100%; max-height:300px; object-fit:contain;" loading="lazy">
                @elseif($ext === 'pdf')
                    <iframe src="{{ asset($file) }}" frameborder="0" width="100%" height="300px" style="border:1px solid #ccc;"></iframe>
                @else
                    <p>Format file tidak didukung</p>
                @endif
