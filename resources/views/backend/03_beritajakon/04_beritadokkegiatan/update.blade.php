...
<section style="background: #FFFFFF; width: 100%; min-height: 100vh;">
        ...
        {{-- ======================================================= --}}
                    <div class="col-md-12">
                        <!--begin::Quick Example-->
                        <form action="{{ route('update.beberitakegiatanjakonnew', $data->id) }}" method="POST" enctype="multipart/form-data">
                                    @csrf
                                    @method('POST')
                                                            <!-- begin::Body -->
                            <div class="card-body">
...
<div class="form-modern col-md-6 mb-3">
    <label class="form-label-modern" for="kegiatanjaskon_id">
        <i class="bi bi-link-45deg" style="margin-right:8px; color:navy;"></i> Kegiatan Jasa Konstruksi
    </label>

    <select id="kegiatanjaskon_id" name="kegiatanjaskon_id"
            class="form-control @error('kegiatanjaskon_id') is-invalid @enderror">

        <!-- Menampilkan data lama (dipilih otomatis) -->
        <option value="{{ $data->kegiatanjaskon_id }}" selected>
            {{ $data->kegiatanjaskon->judul_kegiatan ?? 'Tidak ada judul' }}
        </option>

        <!-- Opsi kegiatan lain -->
        @foreach ($datakegiatan as $k)
            @if ($k->id != $data->kegiatanjaskon_id)
                <option value="{{ $k->id }}">
                    {{ $k->judul_kegiatan }}
                </option>
            @endif
...
<div class="form-modern col-md-6 mb-3">
    <label class="form-label-modern">
        <i class="bi bi-person" style="margin-right:8px; color:navy;"></i> User
    </label>
...
<div class="form-modern col-md-6 mb-3">
    ...
    <textarea id="judul_kegiatan" name="judul_kegiatan"
              class="form-control @error('judul_kegiatan') is-invalid @enderror"
              rows="3">{{ old('judul_kegiatan', $data->judul_kegiatan) }}</textarea>
    ...
    <textarea id="jabatan" name="jabatan"
              class="form-control @error('jabatan') is-invalid @enderror"
              rows="3">{{ old('jabatan', $data->jabatan) }}</textarea>
    ...
    <div class="form-modern col-md-6 mb-3">
        ...
        <input type="file" id="gambar" name="gambar"
               ...
               accept="image/*"
               onchange="previewImage(event, 'previewGambar')">
        ...
        <div class="mt-2">
            <p>Preview Baru:</p>
            <img id="previewGambar" style="width:100%; max-height:200px; object-fit:contain; display:none;">
        </div>
    </div>
    ...
    <div class="form-modern col-12 mb-3">
        <label class="form-label-modern" for="keterangan_berita">
            <i class="bi bi-textarea-resize" style="margin-right:8px; color:navy;"></i> Keterangan Berita
        </label>
        <textarea id="keterangan_berita" name="keterangan_berita"
                  class="form-control @error('keterangan_berita') is-invalid @enderror"
                  rows="15">{{ old('keterangan_berita', $data->keterangan_berita) }}</textarea>
